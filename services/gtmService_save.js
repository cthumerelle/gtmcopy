 ...
 */
const getCopyHistory = async (googleUserId) => {
  try {
    ...
    try {
      ...
      await prisma.copyHistory.create({
        data: {
          ...
          elementTypes: JSON.stringify(elementTypes),
          elementCount: copiedElements.length,
          status: operationStatus,
          message: errors.length > 0 ? JSON.stringify(errors) : null
        }
      });
